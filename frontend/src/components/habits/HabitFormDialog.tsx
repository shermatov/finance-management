...
export function HabitFormDialog({
  ...
  const schema = z.object({ title: z.string().min(1, t("validation.required")).max(200) });
  ...
  useEffect(() => {
    if (open) reset({ title: habit?.title ?? "" });
  }, [open, habit, reset]);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      if (isEditing && habit) {
        await updateHabit.mutateAsync({ id: habit.id, input: { title: values.title } });
        toast.success(t("habits.updated"));
      } else {
        await createHabit.mutateAsync({ title: values.title });
